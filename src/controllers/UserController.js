...
const addAdmin = async function (req, res) {
    ...
    let user, password, responseData, token, verificationCode;
    try {
        user = await isUnique(User, { email });
        if (!user) {
            return errorResponse(res, {
        ...
        password = await hashPassword(last_name.toLowerCase());
        user = {
            first_name,
            last_name,
            email,
...
const viewAdmin = async function (req, res) {
    ...
    try {
        user = await fetch(User, { role: "admin"});
        responseData = {
            payload: user,
            statusCode: 200,
...
const deleteAdmin = async function (req, res) {
    ...
    let { id } = req.params;
    try {
        user = await deleteItem(User, { _id:id, role: "admin"});
        responseData = {
            payload: user,
            statusCode: 200,
...
const viewUser = async function (req, res) {
    let user, responseData;
    try {
        user = await fetch(User, { role: "user"});
        responseData = {
            payload: user,
            statusCode: 200,
            message: "Admin fetched sucessfully.",
